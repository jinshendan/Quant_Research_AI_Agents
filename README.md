...
- Dashboard 一键运行每日研究 pipeline：选择配置文件后从前端直接执行。
...
打开后进入 `一键运行 / One-click Run` 页签，默认会优先读取
`tmp/yinlun_daily.json`；如果没有这个本地文件，会使用
`configs/yinlun_daily.example.json`。确认配置路径后点击 `运行每日研究` 即可。
...
可以直接用仓库里的 `configs/yinlun_daily.example.json` 作为起点。若要保存自己的本地配置，
建议复制到 `tmp/yinlun_daily.json` 后再修改：

```bash
mkdir -p tmp
cp configs/yinlun_daily.example.json tmp/yinlun_daily.json
```
...
推荐方式：打开 dashboard 的 `一键运行 / One-click Run` 页签，点击
`运行每日研究`。
...
- run the daily research pipeline from the dashboard with one click;
...
Use the `One-click Run` tab to run a daily research config directly from the
frontend. The dashboard prefers `tmp/yinlun_daily.json` when it exists and falls
back to `configs/yinlun_daily.example.json`.
...
Recommended: use the dashboard `One-click Run` tab.
